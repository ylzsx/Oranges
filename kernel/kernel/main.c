...
PUBLIC int kernel_main() {
    ...
    for (i = 0; i < NR_TASKS + NR_PROCS; i++) {
        if (i < NR_TASKS) {     // 任务
            ...
            rpl = RPL_USER;
            eflags = 0x202;     // IF = 1, bit 2 is always 1. 剥夺用户进程所有IO权限
        }
        ...
        p_proc->ldts[0].attr1 = DA_C | (privilege << 5);     // 修改DPL=1
        memcpy(&p_proc->ldts[1], &gdt[SELECTOR_KERNEL_DS >> 3], sizeof(DESCRIPTOR));
        p_proc->ldts[1].attr1 = DA_DRW | (privilege << 5);
        p_proc->regs.cs = ((8 * 0) & SA_RPL_MASK & SA_TI_MASK) | SA_TIL | rpl;   // 0000 | 0100 | 0001 = 0101
        p_proc->regs.ds = ((8 * 1) & SA_RPL_MASK & SA_TI_MASK) | SA_TIL | rpl;   // 1000 | 0100 | 0001 = 1101
